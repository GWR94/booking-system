...
The Short Grass booking system is built using Next.js 16 as the frontend framework, React 19 as the UI library, TypeScript as the programming language, Prisma (PostgreSQL) for database management, and Stripe for payment processing.
...
The request flow for The Short Grass booking system is as follows:
...
The Short Grass booking system integrates with the following services:
...
The data model for The Short Grass booking system is defined in `prisma/schema.prisma`. Core models include User, Bay, Slot, and Booking, plus membership fields on `User`.
...
The Short Grass booking system uses the following enums:
...
The Short Grass booking system has the following API routes:
