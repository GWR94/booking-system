...
describe('DesktopSlot Component', () => {
	...
	it('renders "Add to Basket" button when not in basket', () => {
		render(<DesktopSlot {...defaultProps} />, { wrapper: createWrapper() });
		expect(
	...
	it('renders "Add Another" and "Checkout" when slots are in basket', () => {
		render(
			<DesktopSlot
				{...defaultProps}
				basketCount={1}
		...
		expect(screen.getByText(/1 in basket/i)).toBeInTheDocument();
	});
	...
	it('renders "Manage Slot" for admin', () => {
		...
		render(<DesktopSlot {...defaultProps} />, { wrapper: createWrapper() });
		expect(
			screen.getByRole('button', { name: /Manage Slot/i }),
		).toBeInTheDocument();
	});

	it('calls setAdminDialogOpen when "Manage Slot" is clicked', () => {
		(useAuth as any).mockReturnValue({
			user: { role: 'ADMIN' },
			isAdmin: true,
		});
		render(<DesktopSlot {...defaultProps} />, { wrapper: createWrapper() });
		fireEvent.click(screen.getByRole('button', { name: /Manage Slot/i }));
		expect(mockSetAdminDialogOpen).toHaveBeenCalledWith(true);
	});
